        ...
        <tbody>
          {users?.map((u: any) => (
            <tr key={u.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
              ...
              <td style={{ padding: '10px 12px', fontSize: 12, color: '#888' }}>{u.email || '—'}</td>
              ...
              <td style={{ padding: '10px 12px', fontSize: 11, color: '#666' }}>{u.created_at ? new Date(u.created_at).toLocaleDateString() : '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
